...
function updateIntermediateCheckpoint(key, entry, now = Date.now(), force = false) {
  ...
  if (jsonlPath) {
    ...
    debugLog(`inferProgress: key=${key} completedSteps=${JSON.stringify(inferred.completedSteps)} remainingSteps=${JSON.stringify(inferred.remainingSteps)} lastToolCall=${inferred.lastToolCall}`);
    ...
    if (inferred.completedSteps.length > 0) {
      if (!hasExistingSteps || stepsAreConsistent(intermediateProgress.completedSteps, inferred.completedSteps)) {
        ...
        debugLog(`stepsAreConsistent=false, keeping existing steps: ${JSON.stringify(intermediateProgress.completedSteps)}`);
      }
    }
